...
import Select, { Options, components, createFilter } from "react-select";
...
export interface DropdownComponentProps extends VibeComponentProps {
   ...
   */
  defaultValue: DropdownDefaultValue[] | DropdownDefaultValue;
  /**
  ...
  */
   ...
   */
  isOptionSelected: (option: unknown, selectValue: Options<unknown>) => boolean;
  /**
  ...
  (
    ...
    const inlineStyles = useMemo(() => {
      ...
      const mergedStyles = Object.entries(customStyles).reduce((accumulator, [stylesGroup, stylesFn]) => {
        return {
          ...accumulator,
          [stylesGroup]: (defaultStyles: string, state: Record<string, string>) => {
    ...
    const Input = useCallback((props: any) => <components.Input {...props} aria-label="Dropdown input" />, []);
    ...
    const onOptionRemove = useMemo(() => {
      return function (optionValue: string, e: React.MouseEvent | React.KeyboardEvent) {
    ...
    const closeMenuOnScroll = useCallback(
      (event: React.FocusEvent): ((event: React.FocusEvent) => boolean) | boolean => {
